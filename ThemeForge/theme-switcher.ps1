...
$lightWallpaper = Join-Path $scriptDir "wallpaper\ROGDARE.jpg"
$darkWallpaper = Join-Path $scriptDir "wallpaper\ROG G15.png"
$soundPath = Join-Path $scriptDir "sounds\nalak.wav"

# --- Check if Wallpaper and Sound Files Exist ---
if (-not (Test-Path $lightWallpaper)) {
    Write-Host "`n❌ Light wallpaper not found: $lightWallpaper" -ForegroundColor Red
    exit
}
if (-not (Test-Path $darkWallpaper)) {
    Write-Host "`n❌ Dark wallpaper not found: $darkWallpaper" -ForegroundColor Red
    exit
}
...
if (-not (Test-Path $lightWallpaper)) {
    Write-Host "❌ Light wallpaper not found: $lightWallpaper" -ForegroundColor Red
    exit
}
if (-not (Test-Path $darkWallpaper)) {
    Write-Host "❌ Dark wallpaper not found: $darkWallpaper" -ForegroundColor Red
    exit
}
if (-not (Test-Path $soundPath)) {
...
Show-LoadingBar "Changing Theme..." 30 2
...
Show-LoadingBar "Setting Wallpaper..." 30 2
$code = @"
...
Show-LoadingBar "Setting Lock Screen..." 30 2
try {
...
Show-LoadingBar "Reloading Explorer..." 30 2
